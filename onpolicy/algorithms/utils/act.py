...
class ACTLayer(nn.Module):
    ...
    def forward(self, x, available_actions=None, available_actions_first=None, available_actions_second=None, deterministic=False):
        if self.grid_goal or self.grid_goal_simpler:
            ...
            for i, action_out in enumerate(self.action_outs):
                if i == 0:
                    action_logit = action_out(x, available_actions_first)
                else:
                    new_available_actions_second = torch.zeros_like(available_actions_second[:,0])
                    for j in range(action.shape[0]):
                        new_available_actions_second[j] = available_actions_second[j,int(action[j])]
                    action_logit = action_out(x, new_available_actions_second)
                action = action_logit.mode() if deterministic else action_logit.sample()
                action_log_prob = action_logit.log_probs(action)
    ...
    def evaluate_actions(self, x, action, available_actions=None, available_actions_first=None, available_actions_second= None, active_masks=None):
        if self.grid_goal or self.grid_goal_simpler:
            x = self.to_input(x)
            ...
            for action_out, act in zip(self.action_outs, action):
                if i ==0:
                    ...
                    for j in range(act.shape[0]):
                        new_available_actions_second[j] = available_actions_second[j, int(act[j])]
                    action_logit = action_out(x, new_available_actions_second)
                i+=1
                action_log_probs.append(action_logit.log_probs(act))
                if active_masks is not None:
