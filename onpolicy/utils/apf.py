    ...
    def schedule(self, map, unexplored, locations, steps, agent_id, penalty = None, full_path = True, clear_disk = False, random_goal = False):
        ...
        for cluster in clusters:
            ...
            while len(que) > 0:
                ...
                for dx, dy in steps:
                    x1 = x + dx
                    y1 = y + dy
                    if x1>=0 and x1<H and y1>=0 and y1<W and dis[x1, y1] == 1e9 and map[x1, y1] in [0,2]:
                        dis[x1, y1] = dis[x, y]+1
                        que.append((x1,y1))
            dis[sx, sy] = 1e9
            dis = 1 / dis
        ...
        while it <= self.num_iters and minDis2Target > 1:
            ...
            for dx, dy in steps:
                ...
                if neighbor_loc[0]>=0 and neighbor_loc[0]<H and neighbor_loc[1]>=0 and neighbor_loc[1]<W and map[neighbor_loc[0], neighbor_loc[1]] == 1:
                    continue
                if neighbor_loc[0] >= H or neighbor_loc[1] >= W:
                    continue
