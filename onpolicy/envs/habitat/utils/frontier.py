...
def nearest_frontier(map, unexplored, locations, steps, agent_id, clear_radius = 40, cluster_radius = 5, random_goal = False, training = False):
    ...
    while len(que)>0:
        ...
        for i in range(len(neighbors)):
            temp = (max(neighbors[i][0],0), max(neighbors[i][1], 0))
            temp = (min(temp[0],H-1), min(temp[1], W-1))
            neighbors[i] = (temp)
        for u,v in neighbors:
            if u>=0 and u<H and v>=0 and v<W and dis[u, v] == 1e9 and map[u,v] in [0,2] and vis[u,v] == 0:
                vis[u,v] = 1
                dis[u,v] = dis[x,y] + 1
                que.append((u,v))
    min_dis = 1e9
        ...
        if training:
            ...
            if d > clear_radius:
                if min_dis > dis[p[0], p[1]]:
                    min_x, min_y = p
                    min_dis = dis[p[0], p[1]]
    if (min_x == None and not training) or (training and len(goals_for_training) == 0):
        # no valid target
        if random_goal:
            ...
            if training:
                goals_for_training.append(locations[agent_id])
            else:
                min_x, min_y = locations[agent_id][0], locations[agent_id][1]
    if training:
        return goals_for_training
    else:
        return min_x, min_y
...
def ILP_frontier(map, unexplored, locations,  clear_radius = 40, cluster_radius = 5, IG_radius = 50, pre_goals = None, goal_mask = None, random_goal = False):
    H, W = map.shape
    num_agents = len(locations)
    ...
    unexplored = unexplored.copy().astype(np.int32)
    goals = np.zeros((num_agents, 2), dtype = np.int32)
    #goals_u = np.zeros((num_agents, 1), dtype = np.int32)
    ...
    if goal_mask == None:
        ...
        for agent_id in range(num_agents):
            if goal_mask[agent_id]:
                mat = circle_matrix(H, W, pre_goals[agent_id], IG_radius)
                unexplored[mat == 1] = 0
    ...
    num_clusters = len(clusters)
    # compute utility
    cluster_u = [[] for i in range(num_agents)]
    cluster_p = [[] for i in range(num_agents)]

    for it, cluster in enumerate(clusters):
        p = cluster['center']
        if max([l2distance(p, locations[i]) for i in range(num_agents)]) <= clear_radius:
            continue
        mat = circle_matrix(H, W, p, IG_radius) 
        tmp = unexplored[mat == 1].sum() #IG
        d_min = np.inf
        for i in range(num_agents):
            d = l2distance(p, locations[i])
            if d_min > d:
                num_id = i
                d_min = d
            u_v = tmp/(d_min**2)
            
        cluster_p[num_id].append(p)
        cluster_u[num_id].append(u_v)
    for i in range(num_agents):
        zipped = zip(cluster_u[i], cluster_p[i])
        sort_zipped = sorted(zipped,key=lambda x:(x[0]))
        result = zip(*sort_zipped)
        x_axis, y_axis = [list(x) for x in result]
        goals[i] = y_axis[0]
        # goals_u[i] = x_axis[0]
        # cluster_u[i] = x_axis.pop(0)
        # cluster_p[i] = y_axis.pop(0)
        # for j in range(i):
        #     if goals[i] == goals[j]:
        #         if cluster_u[i][0]+goals_u[j] > cluster_u[j][0]+goals_u[i]:
        #             goals[i] = cluster_p[i][0]
        #             goals_u[i] = cluster_u[i][0]
        #             cluster_u[i].pop(0)
        #             cluster_p[i].pop(0)
        #         else:
        #             goals[j] = cluster_p[j][0]
        #             goals_u[j] = cluster_u[j][0]
        #             cluster_u[j].pop(0)
        #             cluster_p[j].pop(0)
    return goals
