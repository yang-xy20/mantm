...
class Runner(object):
    def __init__(self, config):
        ...
        self.direct_action_dir = self.all_args.direct_action_dir
        ...
        if "ft" not in self.algorithm_name:
            ...
            self.buffer = SharedReplayBuffer(self.all_args,
                                            ...
                                            share_observation_space,
                                            self.envs.action_space[0])
        if self.direct_action_dir != '0':
            all_args = copy.deepcopy(self.all_args)
            all_args.use_num_embedding = False
            all_args.use_action_embedding = True
            all_args.use_each_node = False
            all_args.use_recurrent_policy = True
            all_args.use_goal_embedding = False
            self.direct_action_policy = Policy(all_args,
                                self.envs.observation_space[0],
                                share_observation_space,
                                gym.spaces.Discrete(3),
                                device = self.device)

            policy_actor_state_dict = torch.load(str(self.direct_action_dir) + '/actor.pt', map_location=self.device)
            self.direct_action_policy.actor.load_state_dict(policy_actor_state_dict)
            self.direct_action_trainer = TrainAlgo(all_args, self.direct_action_policy, device = self.device)
